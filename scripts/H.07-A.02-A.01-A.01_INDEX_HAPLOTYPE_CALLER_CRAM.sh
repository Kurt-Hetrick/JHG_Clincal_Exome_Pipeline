...
START_INDEX_HC_CRAM=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD="singularity exec $ALIGNMENT_CONTAINER samtools" \
			CMD=$CMD" index" \
				CMD=$CMD" $CORE_PATH/$PROJECT/$FAMILY/$SM_TAG/HC_CRAM/$SM_TAG".HC.cram""
...
END_INDEX_HC_CRAM=`date '+%s'` # capture time process stops for wall clock tracking purposes.
...
# make a copy/rename the cram index file since their appears to be two useable standards
	...
	cp -rvf $CORE_PATH/$PROJECT/$FAMILY/$SM_TAG/HC_CRAM/$SM_TAG".HC.cram.crai" \
	$CORE_PATH/$PROJECT/$FAMILY/$SM_TAG/HC_CRAM/$SM_TAG".HC.crai"
