...
START_FILTER_TO_SAMPLE_ALL_SITES_TARGET=`date '+%s'`
	...
	# construct command line
		...
		CMD="singularity exec $ALIGNMENT_CONTAINER bedtools"
			CMD=$CMD" intersect"
				CMD=$CMD" -header"
				CMD=$CMD" -a $CORE_PATH/$PROJECT/$FAMILY/$SM_TAG/VCF/FILTERED_ON_BAIT/${SM_TAG}.ALL_SITES.vcf.gz"
				CMD=$CMD" -b $CORE_PATH/$PROJECT/TEMP/${SM_TAG}_${TARGET_BED_NAME}-${PADDING_LENGTH}-BP-PAD.bed"
		CMD=$CMD" | singularity exec $ALIGNMENT_CONTAINER bgzip"
			CMD=$CMD" -@ $THREADS"
			CMD=$CMD" -c"
		CMD=$CMD" >| $CORE_PATH/$PROJECT/$FAMILY/$SM_TAG/VCF/FILTERED_ON_TARGET/${SM_TAG}.ALL_SITES.TARGET.vcf.gz"
			CMD=$CMD" &&"
...
# write out timing metrics to file
	...
	echo ${FAMILY}_${PROJECT},S.01,FILTER_TO_SAMPLE_ALL_SITES_TARGET,${HOSTNAME},${START_FILTER_TO_SAMPLE_ALL_SITES_TARGET},${END_FILTER_TO_SAMPLE_ALL_SITES_TARGET} \
