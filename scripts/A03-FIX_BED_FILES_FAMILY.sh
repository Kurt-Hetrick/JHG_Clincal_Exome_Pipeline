...
# THIS IS PADDED WITH 250 BP AND THEN MERGED FOR OVERLAPPING REGIONS.
	...
	cat ${CORE_PATH}/${PROJECT}/TEMP/${FAMILY}-${CODING_BED_NAME}-${CODING_MD5}.bed \
	${CORE_PATH}/${PROJECT}/TEMP/${FAMILY}-${BAIT_BED_NAME}.bed \
		| sort \
			-k 1,1 \
			-k 2,2n \
		...
		| singularity exec ${ALIGNMENT_CONTAINER} bedtools \
			merge \
			-i - \
	>| ${CORE_PATH}/${PROJECT}/TEMP/${FAMILY}-${BAIT_BED_NAME}-${CODING_BED_NAME}-${CODING_MD5}-${GVCF_PAD}-BP-PAD-GVCF.bed
...
# print CHROMOSOME, START, END, ARM (TAB DELIMITED) TO MAKE A BED FILE.
	...
	sed 's/^chr//g' ${CYTOBAND_BED} \
		...
		| awk 'BEGIN {OFS="\t"} \
			{print $1,$3,$4,$2}' \
	>| ${CORE_PATH}/${PROJECT}/TEMP/${FAMILY}.CHROM_ARM.bed
...
# ti/tv bed is used as the target since it shouldn't change
	...
	# bait bed
		...
		(grep "^@SQ" ${REF_DIR}/${REF_BASENAME}.dict \
			...
			${CORE_PATH}/${PROJECT}/TEMP/${FAMILY}-${BAIT_BED_NAME}.bed) \
		>| ${CORE_PATH}/${PROJECT}/TEMP/${FAMILY}-${BAIT_BED_NAME}-picard.bed
	...
	# target-TITV bed
		...
		(grep "^@SQ" ${REF_DIR}/${REF_BASENAME}.dict \
			; awk 'BEGIN {OFS="\t"} \
				{print $1,($2+1),$3,"+",$1"_"($2+1)"_"$3}' \
			${CORE_PATH}/${PROJECT}/TEMP/${FAMILY}-${TITV_BED_NAME}.bed) \
		>| ${CORE_PATH}/${PROJECT}/TEMP/${FAMILY}-${TITV_BED_NAME}-picard.bed
