...
START_DOC_TARGET=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD=${CMD}" --analysis_type DepthOfCoverage"
			...
			CMD=${CMD}" --intervals ${CORE_PATH}/${PROJECT}/TEMP/${SM_TAG}_${TARGET_BED_NAME}-${PADDING_LENGTH}-BP-PAD.bed"
		...
		CMD=${CMD}" --out ${CORE_PATH}/${PROJECT}/${FAMILY}/${SM_TAG}/REPORTS/DEPTH_OF_COVERAGE/TARGET_PADDED/${SM_TAG}_${TARGET_BED_NAME}-${PADDING_LENGTH}-BP-PAD"
