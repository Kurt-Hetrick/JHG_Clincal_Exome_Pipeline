...
START_DECOMPOSE=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD="sed 's/ID=AD,Number=./ID=AD,Number=R/'" \
			CMD=${CMD}" ${CORE_PATH}/${PROJECT}/${FAMILY}/${SM_TAG}/VCF/FILTERED_ON_TARGET/${SM_TAG}.VARIANT_SITES.TARGET.vcf" \
		CMD=${CMD}" | singularity exec ${VT_CONTAINER} vt" \
			CMD=${CMD}" decompose -s -" \
		CMD=${CMD}" | singularity exec ${VT_CONTAINER} vt" \
			CMD=${CMD}" normalize" \
			CMD=${CMD}" -r $REF_GENOME -" \
		CMD=${CMD}" >| ${CORE_PATH}/${PROJECT}/TEMP/${SM_TAG}.VARIANT_SITES.TARGET.DandN.vcf" \
...
# write out timing metrics to file
	...
	echo ${PROJECT},S01,DECOMPOSE,${HOSTNAME},${START_DECOMPOSE},${END_DECOMPOSE} \
