...
START_FILTER_TO_SAMPLE_VARIANT_ONLY=`date '+%s'`
	...
	# construct command line
		...
		CMD=$CMD" --analysis_type SelectVariants"
			...
			CMD=$CMD" --variant $CORE_PATH/$PROJECT/$FAMILY/$SM_TAG/VCF/FILTERED_ON_BAIT/${SM_TAG}.ALL_SITES.vcf.gz"
			CMD=$CMD" --out $CORE_PATH/$PROJECT/$FAMILY/$SM_TAG/VCF/FILTERED_ON_BAIT/${SM_TAG}.VARIANT_SITES.vcf"
...
# write out timing metrics to file
	...
	echo ${FAMILY}_${PROJECT},S.01,FILTER_TO_SAMPLE_VARIANT_ONLY,${HOSTNAME},${START_FILTER_TO_SAMPLE_VARIANT_ONLY},${END_FILTER_TO_SAMPLE_VARIANT_ONLY} \
